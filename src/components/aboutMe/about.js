...
const services = [
  { text: "🔒 Secure API & backend architecture", className: "about-service about-service-blue" },
  { text: "🛡️ Security reviews & threat modeling", className: "about-service about-service-blue" },
  { text: "🧠 System design & scalability consulting", className: "about-service about-service-pink" },
];
  ...
  useEffect(() => {
    ...
    if (typingRef.current.line < lines.length) {
      ...
      if (!currTyped[typingRef.current.line]) {
        currTyped[typingRef.current.line] = { prompt: '', command: '', output: [] };
      }
      if (!typingRef.current.isOutput) {
        ...
        if (typingRef.current.char < curr.command.length) {
          ...
          currTyped[typingRef.current.line].command = curr.command.slice(0, typingRef.current.char + 1);
          ...
          timeout = setTimeout(() => setTypedLines([...currTyped]), afterCommandDelay);
        }
      } else {
        // Typing output
        if (curr.output[typingRef.current.output] === 'services') {
          ...
          timeout = setTimeout(() => setTypedLines([...currTyped]), afterLineDelay);
        } else if (typingRef.current.output < curr.output.length) {
          if (!currTyped[typingRef.current.line].output[typingRef.current.output]) {
            currTyped[typingRef.current.line].output[typingRef.current.output] = '';
          }
          if (typingRef.current.outputChar < curr.output[typingRef.current.output].length) {
            currTyped[typingRef.current.line].output[typingRef.current.output] =
              curr.output[typingRef.current.output].slice(0, typingRef.current.outputChar + 1);
            setTypedLines(currTyped);
            typingRef.current.outputChar++;
            timeout = setTimeout(() => setTypedLines([...currTyped]), outputCharDelay);
          } else {
            typingRef.current.output++;
            typingRef.current.outputChar = 0;
            timeout = setTimeout(() => setTypedLines([...currTyped]), afterOutputDelay);
          }
        } else {
          typingRef.current.line++;
          typingRef.current.char = 0;
  ...
  return (
    ...
    <div className="about-terminal-container">
      <div className="about-terminal-header">
        ...
        <span className="about-terminal-title">Terminal — Accessing Fahad Iqbal</span>
      </div>
      <div className="about-terminal-body">
        {typedLines.map((line, idx) => (
          <div className="about-terminal-row" key={idx}>
            <span className="about-terminal-user">{terminalLines[idx].prompt}</span>{' '}
            <span className="about-terminal-cmd">{line.command}</span>
            {line.command.length === terminalLines[idx].command.length && line.output && line.output.length > 0 && (
              <>
                {line.output[0] === 'services' ? (
                  <div className="about-terminal-services">
                    {services.map((s, i) => (
                      <span className={s.className} key={i}>{s.text}</span>
                    ))}
                  </div>
                ) : line.output[0] && line.output[0].startsWith('> Node.js') ? (
                  <div className="about-terminal-skills">
                    <div className="about-skill about-skill-node">Node.js</div>
                    <div className="about-skill about-skill-mysql">MySql</div>
                    <div className="about-skill about-skill-postgresql">Postgresql</div>
                    <div className="about-skill about-skill-express">Express.js</div>
                    <div className="about-skill about-skill-fastify">Fastify.js</div>
                    <div className="about-skill about-skill-queue">Queue Management</div>
